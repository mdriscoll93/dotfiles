...
# Path to the Rocky Linux cloud image (make sure to download the correct one)
CLOUD_IMG="/var/lib/libvirt/images/Rocky-9-GenericCloud.qcow2"
...
# Download the Rocky Linux cloud image if it doesn't already exist
if [ ! -f "$CLOUD_IMG" ]; then
    echo "Downloading Rocky Linux 9 cloud image..."
    wget https://dl.rockylinux.org/pub/rocky/9/images/x86_64/Rocky-9-GenericCloud.latest.x86_64.qcow2 -O $CLOUD_IMG
fi

# Create a new disk image for the VM
...
users:
  ...
  - name: rocky
    ...
    ssh_pwauth: True
    lock_passwd: False
    password: $(openssl passwd -1 "rocky")
